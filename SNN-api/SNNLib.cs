...
namespace snn
{
    public class SNNLib
    {
        ...
        public apiResponse logMeIn(Dictionary<string, string> inputUser, HttpContext myContext = null)
        {
            ...
            user = allmatching.Where(u => u.password == inputUser["password"] || u.password == clib.encrypt(inputUser["password"]) || u.password == clib.hashMe(inputUser["password"], u.token.ToString())).FirstOrDefault();
            if (user != null)
            {
