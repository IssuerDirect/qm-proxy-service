...
$(function () {
    eventlist = new Vue({
        ...
        data: {
            ...
            selectAll: false,
            msgBox: "",
            totalCount: eventsListData.count,
            currentPage: 0
        },
        computed: {
            title: function () { 
        ...
        methods: {
            ...
            search: async function () {
                ...
                var items = await (await net3000.common.handlePromise({
                    apiurl: `/admin/events/?pageIndex=0&keywords=${this.keywords}&json=true`
                })).json();
            ...
            loadNextPage: async function () {
                ...
                var nextPage = await (await net3000.common.handlePromise({
                    apiurl: `/admin/events/?pageIndex=${this.currentPage}&keywords=${this.keywords}&json=true`
                })).json();
                this.fullList = this.fullList.concat(nextPage.data);
                this.totalCount = this.fullList.count;
            },
            async deleteevent(item = null) {
                let confirmResult = await Swal.fire({
                    title: 'Are you sure?',
                    text: `You're about to delete event# ${this.recs.length > 0 ? this.recs.join() : item.id}`,
                    icon: 'warning',
                ...
                if (confirmResult.value) {
                    ...
                    var res = await (await net3000.common.handlePromise({ apiurl: `/admin/event?ids=${this.recs.length > 0 ? this.recs.join() : item.id}`, method: "Delete" })).json();
